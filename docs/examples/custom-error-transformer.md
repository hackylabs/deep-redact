...
```json
{ "message": "Request failed with status 401", "data": { "username": "alice", "password": "hunter2" } }
```

## Serialised output

```text
{"name":"RequestError","message":"Request failed with status 401","config":{"data":{"username":"alice","password":"[REDACTED]"}}}
```
