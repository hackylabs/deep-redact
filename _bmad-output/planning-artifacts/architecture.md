...
- Caching is instance-scoped precompilation only. The system caches compiled rules, not user payloads. For exact-path-only configurations, compiled property accessor closures are also cached at init time and reused across all `.redact()` invocations; when the fast lane handles a call its redaction path allocates no per-call WeakMap, frozen array, `Object.defineProperty`, or canonical path string (a lightweight per-call safety guard may run first to decide lane selection, but it allocates none of those structures).
...
- The runtime uses a two-lane execution strategy:
  - a compiled fast lane for exact-path-only configurations: property accessor closures generated at init time that read and write target properties directly with no per-call traversal state allocation, targeting the aspirational `25%–50%` overhead band versus `fast-redact` while staying within the `60%` release-blocking ceiling. Exact-path-only is a per-call *candidacy* condition, not a guarantee: a lightweight per-call guard delegates payloads containing supported-transformable runtime values, non-plain prototypes, circular references, or throwing accessors to the generic lane so observable behaviour stays identical
  - a generic iterative traversal lane for all other configurations: structured selectors, wildcard paths, matcher objects, regex path segments, key matching, substring rules, transformer handling, and hostile-shape recovery
