...
- Caching is instance-scoped precompilation only. The system caches compiled rules, not user payloads. For exact-path-only configurations, compiled property accessor closures are also cached at init time and reused across all `.redact()` invocations with zero per-call allocation overhead.
...
- The runtime uses a two-lane execution strategy:
  - a compiled fast lane for exact-path-only configurations: property accessor closures generated at init time that read and write target properties directly with no per-call traversal state allocation, delivering performance within the `25%–50%` overhead target versus `fast-redact`
