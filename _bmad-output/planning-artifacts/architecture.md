...
The architecture is strongly shaped by non-functional constraints. Performance is release-critical: comparable path-based workloads target roughly 25 to 50 per cent overhead versus `fast-redact` as an aspirational goal, with a release-blocking ceiling of 60 per cent overhead, backed by published benchmark artefacts. Security requirements are equally strong: one-way redaction only, no restore capability, no leaking diagnostic placeholders, and explicit documentation of precedence rules for overlapping targeting modes. Reliability requirements demand that supported inputs do not throw after successful initialisation and that failures are isolated to the problematic nested value rather than collapsing the entire payload.
...
- Runtime execution model: redaction uses a benchmark-led hybrid engine with no user-visible mutation mode. Path-driven configurations use a rule-driven engine that performs a trie-guided single traversal: the compile-time prefix trie is walked top-down against the live payload, following only branches the trie has edges for and applying rules at terminal nodes — shared prefixes are visited exactly once regardless of how many rules pass through them, and unconfigured payload positions are never visited. Configurations containing recursive wildcards (`**`), regex path segments, key rules, substring tests, fuzzy matching, or case-insensitive matching fall back to a generic iterative traversal engine. Both engines must produce the same observable behaviour for matching, censoring, diagnostics, precedence, and `[UNSUPPORTED]` output. **Rule-driven engine:** for configurations selected by the `pathDrivenOnly: true` compile-time flag (exact path segments and/or single-level wildcard `*` segments, with no `**`, regex path segments, key rules, substring tests, fuzzy matching, or case-insensitive matching), the engine navigates directly to each configured target without walking the full payload — O(P) for exact paths, O(P + Σ K_wildcard_levels) for single-wildcard paths — with no per-call WeakMap, frozen path-segment array, or canonical path string allocation. A `Map<object, shallowCopy>` ancestor map ensures shared containers are shallow-copied exactly once. The generic iterative traversal engine is the sole executor for all `pathDrivenOnly: false` configurations. **Implementation status:** as of Epic 7, the exact-path compiled executor (Story 7.1) is complete and the exact-path benchmark gate passes. Epic 8 (Rule-Driven Traversal Engine) supersedes Story 7.1 with the rule-driven engine described above, extending efficient navigation to single-wildcard configurations and deprecating the separate compiled executor. Object-identity tracking is scoped to a single call to `redact` and must distinguish between in-progress traversal and completed traversal. Re-encountering an in-progress identity during the same call follows the circular-reference handling path. Re-encountering a fully traversed object or array during the same call must not trigger a second descent; the runtime should reuse the already resolved transformed result where safe, or otherwise deterministically skip re-entry. A sanitised warning may be emitted optionally, but repeated completed identities are otherwise treated as benign. Each subsequent call to `redact` resets traversal state while retaining the already initialised and compiled configuration in memory.
...
- Transformer contract: v4 includes a built-in default transformer set for circular references, `BigInt`, `Date`, `Error`, `Map`, `RegExp`, `Set`, and `URL`, and allows users to register custom transformers by value type, constructor, or fallback category. Transformer resolution order must be deterministic and documented. Built-in and custom transformers must share the same failure semantics, and any transformer failure must participate in the same `[UNSUPPORTED]` graceful-degradation contract as the rest of the runtime. **Transformation and circular-reference neutralisation are an output-stage concern owned by the serialise adapter (Epic 8 Story 8.3) and applied only when `serialise: true`. Under `serialise: false` (structured output, the default), transformable runtime values and circular references are returned raw; the redaction traversal does not transform them. The transformer set, its deterministic resolution order, and its `[UNSUPPORTED]` failure semantics are defined once and reused by the serialise adapter, preserving a single source of truth.**
...
- Caching is instance-scoped precompilation only. The system caches compiled rules, not user payloads. For `pathDrivenOnly: true` configurations (exact path segments and/or single-level `*` wildcard segments, with no recursive wildcards, regex path segments, key rules, substring tests, fuzzy matching, or case-insensitive matching), the rule-driven engine's compiled rule plan is cached at init time and reused across all `.redact()` invocations with no per-call WeakMap, frozen array, `Object.defineProperty`, or canonical path string allocation.
...
- The runtime uses a two-lane execution strategy:
  - a rule-driven engine for `pathDrivenOnly: true` configurations: performs a trie-guided single traversal — the compile-time prefix trie is walked top-down against the live payload, following only branches the trie has edges for and applying rules at terminal trie nodes, so shared prefixes are visited exactly once and unconfigured payload positions are never touched — achieving O(P) for exact paths and O(P + Σ K_wildcard_levels) for single-wildcard paths, targeting the aspirational `25%–50%` overhead band versus `fast-redact` while staying within the `60%` release-blocking ceiling. A configuration is `pathDrivenOnly: true` when it contains only exact path segments and/or single-level wildcard (`*`) path segments, with no recursive wildcards (`**`), no regex path segments, no ignore rules on paths, no key rules, no substring tests, no fuzzy key matching, and case-sensitive matching enabled. A `Map<object, shallowCopy>` ancestor map ensures shared containers are shallow-copied exactly once when multiple rules target the same subtree. Payloads containing non-plain prototypes at traversed intermediate nodes trigger delegation to the generic engine; all other supported inputs are handled by the rule-driven engine
  - a generic iterative traversal lane for all other configurations: recursive wildcard (`**`) paths, regex path segments, key matching, substring rules, matcher objects, and hostile-shape recovery (runtime-value transformation is **not** a traversal-lane responsibility — it is owned by the serialise output adapter; see below)
...
- Structured and serialised output are both first-class. Serialisation remains a final output-adapter step rather than a separate traversal mode. Runtime-value transformation (Date, BigInt, Map, Set, Error, RegExp, URL) and circular-reference neutralisation are owned by this serialise output adapter and run **only when `serialise: true`** (Epic 8 Story 8.3). Under `serialise: false` (the default), transformable runtime values and circular references are returned raw, exactly as they appear in the input at non-redacted positions; the library never throws during redaction (FR26), and `serialise: false` output is safe to serialise when the input is itself serialisable. Guaranteed `JSON`-safe, non-throwing serialised output — circular circuit-breaking, transformer markers, and per-value `[UNSUPPORTED]` isolation — is delivered by `serialise: true`.
...
4. Implement the rule-driven engine and the generic traversal fallback under one benchmarked runtime. **Status:** the generic traversal (correctness-complete as of Epic 6) and the compiled path executor (Epic 7 Story 7.1, exact-path benchmark gate passing) are complete. Epic 8 supersedes Story 7.1 with the rule-driven engine — eliminating the separate fast lane and extending efficient path-driven navigation to single-wildcard configurations.
