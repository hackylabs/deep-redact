...
- Caching is instance-scoped precompilation only. The system caches compiled rules, not user payloads. For fast-lane-eligible configurations (exact path segments and/or single-level `*` wildcard segments, with no recursive wildcards, regex path segments, key rules, substring tests, fuzzy matching, or case-insensitive matching), compiled property accessor closures and the prefix trie are cached at init time and reused across all `.redact()` invocations; when the fast lane handles a call its redaction path allocates no per-call WeakMap, frozen array, `Object.defineProperty`, or canonical path string (a lightweight per-call safety guard may run first to decide lane selection, but it allocates none of those structures).
...
- The runtime uses a two-lane execution strategy:
  - a compiled fast lane for fast-lane-eligible configurations: property accessor closures and a prefix trie generated at init time that read and write target properties directly with no per-call traversal state allocation, targeting the aspirational `25%–50%` overhead band versus `fast-redact` while staying within the `60%` release-blocking ceiling. A configuration is fast-lane-eligible when it contains only exact path segments and/or single-level wildcard (`*`) path segments, with no recursive wildcards (`**`), no regex path segments, no ignore rules on paths, no key rules, no substring tests, no fuzzy key matching, and case-sensitive matching enabled. The prefix trie includes a `wildcardChild` branch so that a single traversal pass handles both exact and `*.field` paths. Fast-lane eligibility is a per-call *candidacy* condition, not a guarantee: a lightweight per-call guard delegates payloads containing supported-transformable runtime values, non-plain prototypes, circular references, or throwing accessors to the generic lane so observable behaviour stays identical
...
4. Implement the exact-path fast lane and the generic traversal fallback under one benchmarked runtime. **Status:** the generic traversal (correctness-complete as of Epic 6) delivers step 4 partially. The compiled property accessor fast lane — required for the benchmark gate — is deferred to Epic 7 Story 7.1.
