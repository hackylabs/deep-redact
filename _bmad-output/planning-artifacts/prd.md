...
**Technical Risks:** The primary technical risk is performance recovery. v4 must preserve and extend capability breadth while recovering enough runtime performance to remain credible against `fast-redact`. Mitigation requires benchmark-led development, published artefacts, and aggressive focus on hot-path costs during design and implementation. This risk has partially materialised: the v4.0.0 benchmark artefact for the `path-based-single-object-node24` workload records 5566.4% overhead, against the 25–50% aspirational target and the 60% release ceiling, because the general traversal engine (correctness-complete as of Epic 6) carries per-call allocation costs that dominate on small exact-path workloads. Epic 7 Stories 7.1–7.4 are complete: the compiled path executor (7.1) closes the exact-path benchmark gate; the equivalence corpus (7.2), allocation optimisations (7.3), and traversal safety limits (7.4) are shipped. Story 7.5 — which proposed extending a prefix trie to support single-level wildcard paths in the fast lane — is cancelled, superseded by Epic 8 (Rule-Driven Traversal Engine). Epic 8 inverts the outer traversal loop to navigate directly to configured targets rather than walking the full payload, achieving O(P) for exact paths and O(P + Σ K_wildcard_levels) for single-wildcard configurations without requiring a separate fast lane. The wildcard-workload benchmark gate remains open until Epic 8 Story 8.3 is complete.
...
**Current status (v4.0.0):** The `path-based-single-object-node24` benchmark artefact records `overheadPct: 5566.4` against the `maxOverheadPct: 60` release ceiling, a gap of two orders of magnitude. Epic 7 Stories 7.1–7.4 are complete: the compiled path executor (Story 7.1) passes the exact-path benchmark gate. Epic 8 (Rule-Driven Traversal Engine) is the next remediation step, replacing both the compiled path executor and the general O(N) traversal for path-driven configurations with a rule-driven engine that navigates directly to configured targets (O(P) for exact paths, O(P + Σ K_wildcard_levels) for single-wildcard paths); Story 7.5 is cancelled in favour of this structural fix. The wildcard-workload gate opens once Epic 8 Story 8.3 is complete. The aspirational `25–50%` target is retained as the optimisation goal; the `60%` ceiling is the hard gate. The gate remains blocking for protected-branch and release-candidate runs.
